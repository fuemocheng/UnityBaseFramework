...
namespace UnityBaseFramework.Editor.ResourceTools
{
    public sealed partial class ResourceBuilderController
    {
        ...
        {
            m_ConfigurationPath = Type.GetConfigurationPath<ResourceBuilderConfigPathAttribute>() ?? Utility.Path.GetRegularPath(Path.Combine(Application.dataPath, "BaseFramework/Configs/ResourceBuilder.xml"));
        ...
        public string BaseFrameworkVersion
        {
            get
            {
                return BaseFramework.Version.BaseFrameworkVersion;
            }
        }
        ...
        public event BaseFrameworkAction<int, int> OnLoadingResource = null;

        public event BaseFrameworkAction<int, int> OnLoadingAsset = null;

        public event BaseFrameworkAction OnLoadCompleted = null;

        public event BaseFrameworkAction<int, int> OnAnalyzingAsset = null;

        public event BaseFrameworkAction OnAnalyzeCompleted = null;

        public event BaseFrameworkFunc<string, float, bool> ProcessingAssetBundle = null;

        public event BaseFrameworkFunc<string, float, bool> ProcessingBinary = null;

        public event BaseFrameworkAction<Platform> ProcessResourceComplete = null;

        public event BaseFrameworkAction<string> BuildResourceError = null;
        ...
        public bool Load()
        {
            ...
            try
            {
                ...
                XmlNode xmlRoot = xmlDocument.SelectSingleNode("UnityBaseFramework");
        ...
        public bool Save()
        {
            try
            {
                ...
                XmlElement xmlRoot = xmlDocument.CreateElement("UnityBaseFramework");
        ...
        public bool BuildResources()
        {
            ...
            m_BuildReport.Initialize(BuildReportPath, ProductName, CompanyName, GameIdentifier, BaseFrameworkVersion, UnityVersion, ApplicableGameVersion, InternalResourceVersion,
            ...
            try
            {
                ...
                if (m_BuildEventHandler != null)
                {
                    ...
                    m_BuildEventHandler.OnPreprocessAllPlatforms(ProductName, CompanyName, GameIdentifier, BaseFrameworkVersion, UnityVersion, ApplicableGameVersion, InternalResourceVersion,
                ...
                if (!m_ResourceCollection.Load())
                {
                    ...
                    if (m_BuildEventHandler != null)
                    {
                        ...
                        m_BuildEventHandler.OnPostprocessAllPlatforms(ProductName, CompanyName, GameIdentifier, BaseFrameworkVersion, UnityVersion, ApplicableGameVersion, InternalResourceVersion,
                            Platforms, AssetBundleCompression, CompressionHelperTypeName, AdditionalCompressionSelected, ForceRebuildAssetBundleSelected, BuildEventHandlerTypeName, OutputDirectory, buildAssetBundleOptions,
                            WorkingPath, OutputPackageSelected, OutputPackagePath, OutputFullSelected, OutputFullPath, OutputPackedSelected, OutputPackedPath, BuildReportPath);
                    }
        ...
        private void ProcessPackageVersionList(string outputPackagePath, Platform platform)
        {
            ...
            using (FileStream fileStream = new FileStream(packageVersionListPath, FileMode.Create, FileAccess.Write))
            {
                ...
                {
                    throw new BaseFrameworkException("Serialize package version list failure.");
                }
            }
        }
        ...
        private VersionListData ProcessUpdatableVersionList(string outputFullPath, Platform platform)
        {
            ...
            using (FileStream fileStream = new FileStream(updatableVersionListPath, FileMode.Create, FileAccess.Write))
            {
                if (!serializer.Serialize(fileStream, versionList))
                {
                    throw new BaseFrameworkException("Serialize updatable version list failure.");
                }
            }
        ...
        private void ProcessReadOnlyVersionList(string outputPackedPath, Platform platform)
        {
            ...
            using (FileStream fileStream = new FileStream(readOnlyVersionListPath, FileMode.Create, FileAccess.Write))
            {
                if (!serializer.Serialize(fileStream, versionList))
                {
                    throw new BaseFrameworkException("Serialize read-only version list failure.");
                }
            }
        }
        ...
        private void CreateFileSystems(IEnumerable<ResourceData> resourceDatas, string outputPath, Dictionary<string, IFileSystem> outputFileSystem)
        {
            ...
            {
                m_FileSystemManager = BaseFrameworkEntry.GetModule<IFileSystemManager>();
        ...
        private static BuildTarget GetBuildTarget(Platform platform)
        {
            switch (platform)
            {
                ...
                default:
                    throw new BaseFrameworkException("Platform is invalid.");
            }
        }
