...
namespace BaseFramework.Resource
{
    internal sealed partial class ResourceManager : BaseFrameworkModule, IResourceManager
    {
        ...
        private sealed partial class ResourceChecker
        {
            ...
            public BaseFrameworkAction<ResourceName, string, LoadType, int, int, int, int> ResourceNeedUpdate;
            public BaseFrameworkAction<int, int, int, long, long> ResourceCheckComplete;
            ...
            private void RefreshCheckInfoStatus()
            {
                ...
                foreach (KeyValuePair<ResourceName, CheckInfo> checkInfo in m_CheckInfos)
                {
                    ...
                    else if (ci.Status == CheckInfo.CheckStatus.StorageInReadWrite)
                    {
                        if (ci.NeedMoveToDisk || ci.NeedMoveToFileSystem)
                        {
                            ...
                            if (ci.NeedMoveToDisk)
                            {
                                ...
                                {
                                    throw new BaseFrameworkException(Utility.Text.Format("Save as file '{0}' to '{1}' from file system '{2}' error.", resourceFullName, resourcePath, fileSystem.FullPath));
                                }
                            ...
                            if (ci.NeedMoveToFileSystem)
                            {
                                ...
                                {
                                    throw new BaseFrameworkException(Utility.Text.Format("Write resource '{0}' to file system '{1}' error.", resourceFullName, fileSystem.FullPath));
                                }
                    ...
                    {
                        throw new BaseFrameworkException(Utility.Text.Format("Check resources '{0}' error with unknown status.", ci.ResourceName.FullName));
                    }
            ...
            private void OnLoadUpdatableVersionListSuccess(string fileUri, byte[] bytes, float duration, object userData)
            {
                ...
                {
                    throw new BaseFrameworkException("Updatable version list has been parsed.");
                }
                ...
                try
                {
                    ...
                    {
                        throw new BaseFrameworkException("Deserialize updatable version list failure.");
                    }
                ...
                {
                    if (exception is BaseFrameworkException)
                    {
                        throw;
                    }

                    throw new BaseFrameworkException(Utility.Text.Format("Parse updatable version list exception '{0}'.", exception), exception);
                }
                finally
                {
            ...
            {
                throw new BaseFrameworkException(Utility.Text.Format("Updatable version list '{0}' is invalid, error message is '{1}'.", fileUri, string.IsNullOrEmpty(errorMessage) ? "<Empty>" : errorMessage));
            }
            ...
            private void OnLoadReadOnlyVersionListSuccess(string fileUri, byte[] bytes, float duration, object userData)
            {
                ...
                {
                    throw new BaseFrameworkException("Read-only version list has been parsed.");
                }
                ...
                try
                {
                    ...
                    if (!versionList.IsValid)
                    {
                        throw new BaseFrameworkException("Deserialize read-only version list failure.");
                    }
                ...
                catch (Exception exception)
                {
                    ...
                    throw new BaseFrameworkException(Utility.Text.Format("Parse read-only version list exception '{0}'.", exception), exception);
                }
                finally
                {
                    if (memoryStream != null)
            ...
            private void OnLoadReadWriteVersionListSuccess(string fileUri, byte[] bytes, float duration, object userData)
            {
                ...
                {
                    throw new BaseFrameworkException("Read-write version list has been parsed.");
                }
                ...
                try
                {
                    ...
                    if (!versionList.IsValid)
                    {
                        throw new BaseFrameworkException("Deserialize read-write version list failure.");
                    }
                ...
                catch (Exception exception)
                {
                    ...
                    throw new BaseFrameworkException(Utility.Text.Format("Parse read-write version list exception '{0}'.", exception), exception);
                }
                finally
                {
                    if (memoryStream != null)
