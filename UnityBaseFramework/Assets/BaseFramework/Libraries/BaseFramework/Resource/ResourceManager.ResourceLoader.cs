        ...
        private sealed partial class ResourceLoader
        {
            ...
            public void LoadAsset(string assetName, Type assetType, int priority, LoadAssetCallbacks loadAssetCallbacks, object userData)
            {
                ...
                if (!CheckAsset(assetName, out resourceInfo, out dependencyAssetNames))
                {
                    ...
                    throw new BaseFrameworkException(errorMessage);
                }
            ...
            public void UnloadScene(string sceneAssetName, UnloadSceneCallbacks unloadSceneCallbacks, object userData)
            {
                ...
                {
                    throw new BaseFrameworkException("You must set resource helper first.");
                }
                ...
                {
                    throw new BaseFrameworkException(Utility.Text.Format("Can not find asset of scene '{0}'.", sceneAssetName));
                }
            ...
            public byte[] LoadBinaryFromFileSystem(string binaryAssetName)
            {
                ...
                if (resourceInfo == null)
                {
                    throw new BaseFrameworkException(Utility.Text.Format("Can not load binary '{0}' from file system which is not exist.", binaryAssetName));
                }

                if (!resourceInfo.Ready)
                {
                    throw new BaseFrameworkException(Utility.Text.Format("Can not load binary '{0}' from file system which is not ready.", binaryAssetName));
                }

                if (!resourceInfo.IsLoadFromBinary)
                {
                    throw new BaseFrameworkException(Utility.Text.Format("Can not load binary '{0}' from file system which is not a binary asset.", binaryAssetName));
                }

                if (!resourceInfo.UseFileSystem)
                {
                    throw new BaseFrameworkException(Utility.Text.Format("Can not load binary '{0}' from file system which is not use file system.", binaryAssetName));
                }
            ...
            private bool LoadDependencyAsset(string assetName, int priority, LoadResourceTaskBase mainTask, object userData)
            {
                if (mainTask == null)
                {
                    throw new BaseFrameworkException("Main task is invalid.");
                }
            ...
            private void DefaultDecryptResourceCallback(byte[] bytes, int startIndex, int count, string name, string variant, string extension, bool storageInReadOnly, string fileSystem, byte loadType, int length, int hashCode)
            {
                ...
                switch ((LoadType)loadType)
                {
                    ...
                    default:
                        throw new BaseFrameworkException("Not supported load type when decrypt resource.");
                }
            ...
            private void OnLoadBinarySuccess(string fileUri, byte[] bytes, float duration, object userData)
            {
                ...
                {
                    throw new BaseFrameworkException("Load binary info is invalid.");
                }
