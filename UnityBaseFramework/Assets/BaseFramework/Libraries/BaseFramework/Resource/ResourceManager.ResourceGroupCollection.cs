    ...
    internal sealed partial class ResourceManager : BaseFrameworkModule, IResourceManager
    {
        ...
        private sealed class ResourceGroupCollection : IResourceGroupCollection
        {
            ...
            public ResourceGroupCollection(ResourceGroup[] resourceGroups, Dictionary<ResourceName, ResourceInfo> resourceInfos)
            {
                ...
                {
                    throw new BaseFrameworkException("Resource groups is invalid.");
                }

                if (resourceInfos == null)
                {
                    throw new BaseFrameworkException("Resource infos is invalid.");
                }
                ...
                for (int i = 0; i < lastIndex; i++)
                {
                    ...
                    {
                        throw new BaseFrameworkException(Utility.Text.Format("Resource group index '{0}' is invalid.", i));
                    }
                    ...
                    for (int j = i + 1; j < resourceGroups.Length; j++)
                    {
                        ...
                        {
                            throw new BaseFrameworkException(Utility.Text.Format("Resource group '{0}' duplicated.", resourceGroups[i].Name));
                        }
                    }
                }
                ...
                {
                    throw new BaseFrameworkException(Utility.Text.Format("Resource group index '{0}' is invalid.", lastIndex));
                }
                ...
                foreach (ResourceGroup resourceGroup in m_ResourceGroups)
                {
                    ...
                    foreach (ResourceName resourceName in cachedResourceNames)
                    {
                        ...
                        {
                            throw new BaseFrameworkException(Utility.Text.Format("Resource info '{0}' is invalid.", resourceName.FullName));
                        }
