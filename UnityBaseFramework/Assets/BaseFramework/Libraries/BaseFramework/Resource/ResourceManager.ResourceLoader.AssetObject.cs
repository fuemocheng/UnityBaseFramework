        ...
        private sealed partial class ResourceLoader
        {
            ...
            private sealed class AssetObject : ObjectBase
            {
                ...
                public static AssetObject Create(string name, object target, List<object> dependencyAssets, object resource, IResourceHelper resourceHelper, ResourceLoader resourceLoader)
                {
                    ...
                    {
                        throw new BaseFrameworkException("Dependency assets is invalid.");
                    }

                    if (resource == null)
                    {
                        throw new BaseFrameworkException("Resource is invalid.");
                    }
                ...
                protected internal override void Release(bool isShutdown)
                {
                    if (!isShutdown)
                    {
                        ...
                        {
                            throw new BaseFrameworkException(Utility.Text.Format("Asset target '{0}' reference count is '{1}' larger than 0.", Name, targetReferenceCount));
                        }
                        ...
                        foreach (object dependencyAsset in m_DependencyAssets)
                        {
                            ...
                            {
                                throw new BaseFrameworkException(Utility.Text.Format("Asset target '{0}' dependency asset reference count is invalid.", Name));
                            }
                        }
