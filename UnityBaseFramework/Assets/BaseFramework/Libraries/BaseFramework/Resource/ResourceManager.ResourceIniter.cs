...
namespace BaseFramework.Resource
{
    internal sealed partial class ResourceManager : BaseFrameworkModule, IResourceManager
    {
        ...
        private sealed class ResourceIniter
        {
            ...
            public BaseFrameworkAction ResourceInitComplete;
            ...
            private void OnLoadPackageVersionListSuccess(string fileUri, byte[] bytes, float duration, object userData)
            {
                ...
                try
                {
                    ...
                    {
                        throw new BaseFrameworkException("Deserialize package version list failure.");
                    }
                ...
                catch (Exception exception)
                {
                    ...
                    throw new BaseFrameworkException(Utility.Text.Format("Parse package version list exception '{0}'.", exception), exception);
                }
                finally
                {
            ...
            {
                throw new BaseFrameworkException(Utility.Text.Format("Package version list '{0}' is invalid, error message is '{1}'.", fileUri, string.IsNullOrEmpty(errorMessage) ? "<Empty>" : errorMessage));
            }
        }
    }
}
