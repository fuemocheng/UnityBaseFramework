...
namespace BaseFramework
{
    ...
    internal sealed class DataProvider<T> : IDataProvider<T>
    {
        ...
        public static void EnsureCachedBytesSize(int ensureSize)
        {
            if (ensureSize < 0)
            {
                throw new BaseFrameworkException("Ensure size is invalid.");
            }
        ...
        public void ReadData(string dataAssetName, int priority, object userData)
        {
            ...
            {
                throw new BaseFrameworkException("You must set data provider helper first.");
            }
            ...
            switch (result)
            {
                ...
                case HasAssetResult.BinaryOnFileSystem:
                    ...
                    {
                        throw new BaseFrameworkException(Utility.Text.Format("Load binary '{0}' from file system with internal error.", dataAssetName));
                    }
                    ...
                    try
                    {
                        ...
                        {
                            throw new BaseFrameworkException(Utility.Text.Format("Load data failure in data provider helper, data asset name '{0}'.", dataAssetName));
                        }
                ...
                default:
                    throw new BaseFrameworkException(Utility.Text.Format("Data asset '{0}' is '{1}'.", dataAssetName, result));
            }
        }
        ...
        public bool ParseData(string dataString, object userData)
        {
            if (m_DataProviderHelper == null)
            {
                throw new BaseFrameworkException("You must set data helper first.");
            }

            if (dataString == null)
            {
                throw new BaseFrameworkException("Data string is invalid.");
            }
            ...
            catch (Exception exception)
            {
                ...
                throw new BaseFrameworkException(Utility.Text.Format("Can not parse data string with exception '{0}'.", exception), exception);
            }
        }
        ...
        public bool ParseData(byte[] dataBytes)
        {
            if (dataBytes == null)
            {
                throw new BaseFrameworkException("Data bytes is invalid.");
            }
        ...
        public bool ParseData(byte[] dataBytes, int startIndex, int length, object userData)
        {
            ...
            catch (Exception exception)
            {
                ...
                throw new BaseFrameworkException(Utility.Text.Format("Can not parse data bytes with exception '{0}'.", exception), exception);
            }
        }
        ...
        internal void SetDataProviderHelper(IDataProviderHelper<T> dataProviderHelper)
        {
            ...
            {
                throw new BaseFrameworkException("Data provider helper is invalid.");
            }
