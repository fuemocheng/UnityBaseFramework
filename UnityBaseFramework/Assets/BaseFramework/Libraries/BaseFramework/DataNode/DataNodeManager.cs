...
namespace BaseFramework.DataNode
{
    /// <summary>
    /// 数据结点管理器。
    /// </summary>
    internal sealed partial class DataNodeManager : BaseFrameworkModule, IDataNodeManager
    {
        ...
        public T GetData<T>(string path, IDataNode node) where T : Variable
        {
            ...
            if (current == null)
            {
                throw new BaseFrameworkException(Utility.Text.Format("Data node is not exist, path '{0}', node '{1}'.", path, node != null ? node.FullName : string.Empty));
            }
