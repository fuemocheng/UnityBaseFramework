...
namespace BaseFramework.FileSystem
{
    ...
    /// 文件系统管理器。
    /// </summary>
    internal sealed class FileSystemManager : BaseFrameworkModule, IFileSystemManager
    {
        ...
        public void SetFileSystemHelper(IFileSystemHelper fileSystemHelper)
        {
            ...
            {
                throw new BaseFrameworkException("File system helper is invalid.");
            }
        ...
        public IFileSystem CreateFileSystem(string fullPath, FileSystemAccess access, int maxFileCount, int maxBlockCount)
        {
            ...
            {
                throw new BaseFrameworkException("Access read is invalid.");
            }

            fullPath = Utility.Path.GetRegularPath(fullPath);
            if (m_FileSystems.ContainsKey(fullPath))
            {
                throw new BaseFrameworkException(Utility.Text.Format("File system '{0}' is already exist.", fullPath));
            }

            FileSystemStream fileSystemStream = m_FileSystemHelper.CreateFileSystemStream(fullPath, access, true);
            if (fileSystemStream == null)
            {
                throw new BaseFrameworkException(Utility.Text.Format("Create file system stream for '{0}' failure.", fullPath));
            }

            FileSystem fileSystem = FileSystem.Create(fullPath, access, fileSystemStream, maxFileCount, maxBlockCount);
            if (fileSystem == null)
            {
                throw new BaseFrameworkException(Utility.Text.Format("Create file system '{0}' failure.", fullPath));
            }
        ...
        public IFileSystem LoadFileSystem(string fullPath, FileSystemAccess access)
        {
            ...
            if (fileSystem == null)
            {
                ...
                throw new BaseFrameworkException(Utility.Text.Format("Load file system '{0}' failure.", fullPath));
            }
        ...
        public void DestroyFileSystem(IFileSystem fileSystem, bool deletePhysicalFile)
        {
            if (fileSystem == null)
            {
                throw new BaseFrameworkException("File system is invalid.");
            }
