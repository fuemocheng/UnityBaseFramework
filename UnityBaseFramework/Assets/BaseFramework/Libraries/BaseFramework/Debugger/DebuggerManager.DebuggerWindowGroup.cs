...
namespace BaseFramework.Debugger
{
    internal sealed partial class DebuggerManager : BaseFrameworkModule, IDebuggerManager
    {
        /// <summary>
        /// 调试器窗口组。
        /// </summary>
        private sealed class DebuggerWindowGroup : IDebuggerWindowGroup
        {
            ...
            public void RegisterDebuggerWindow(string path, IDebuggerWindow debuggerWindow)
            {
                if (string.IsNullOrEmpty(path))
                {
                    throw new BaseFrameworkException("Path is invalid.");
                }
                ...
                if (pos < 0 || pos >= path.Length - 1)
                {
                    ...
                    {
                        throw new BaseFrameworkException("Debugger window has been registered.");
                    }
                ...
                else
                {
                    ...
                    if (debuggerWindowGroup == null)
                    {
                        ...
                        {
                            throw new BaseFrameworkException("Debugger window has been registered, can not create debugger window group.");
                        }
