...
namespace BaseFramework.Download
{
    ...
    internal sealed partial class DownloadManager : BaseFrameworkModule, IDownloadManager
    {
        ...
        public int AddDownload(string downloadPath, string downloadUri, string tag, int priority, object userData)
        {
            ...
            {
                throw new BaseFrameworkException("Download path is invalid.");
            }

            if (string.IsNullOrEmpty(downloadUri))
            {
                throw new BaseFrameworkException("Download uri is invalid.");
            }

            if (TotalAgentCount <= 0)
            {
                throw new BaseFrameworkException("You must add download agent first.");
            }
