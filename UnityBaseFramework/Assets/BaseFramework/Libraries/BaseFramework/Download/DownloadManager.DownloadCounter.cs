namespace BaseFramework.Download
{
    internal sealed partial class DownloadManager : BaseFrameworkModule, IDownloadManager
    {
        ...
        {
            private readonly BaseFrameworkLinkedList<DownloadCounterNode> m_DownloadCounterNodes;
            ...
            public DownloadCounter(float updateInterval, float recordInterval)
            {
                ...
                {
                    throw new BaseFrameworkException("Update interval is invalid.");
                }

                if (recordInterval <= 0f)
                {
                    throw new BaseFrameworkException("Record interval is invalid.");
                }

                m_DownloadCounterNodes = new BaseFrameworkLinkedList<DownloadCounterNode>();
