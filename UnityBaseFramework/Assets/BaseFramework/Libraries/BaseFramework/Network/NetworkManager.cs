...
namespace BaseFramework.Network
{
    ...
    internal sealed partial class NetworkManager : BaseFrameworkModule, INetworkManager
    {
        ...
        public INetworkChannel CreateNetworkChannel(string name, ServiceType serviceType, INetworkChannelHelper networkChannelHelper)
        {
            ...
            {
                throw new BaseFrameworkException("Network channel helper is invalid.");
            }

            if (networkChannelHelper.PacketHeaderLength < 0)
            {
                throw new BaseFrameworkException("Packet header length is invalid.");
            }

            if (HasNetworkChannel(name))
            {
                throw new BaseFrameworkException(Utility.Text.Format("Already exist network channel '{0}'.", name ?? string.Empty));
            }
            ...
            switch (serviceType)
            {
                ...
                default:
                    throw new BaseFrameworkException(Utility.Text.Format("Not supported service type '{0}'.", serviceType));
            }
