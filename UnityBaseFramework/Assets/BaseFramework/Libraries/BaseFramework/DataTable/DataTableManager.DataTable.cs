...
namespace BaseFramework.DataTable
{
    internal sealed partial class DataTableManager : BaseFrameworkModule, IDataTableManager
    {
        /// <summary>
        /// 数据表。
        /// </summary>
        ...
        private sealed class DataTable<T> : DataTableBase, IDataTable<T> where T : class, IDataRow, new()
        {
            ...
            public bool HasDataRow(Predicate<T> condition)
            {
                if (condition == null)
                {
                    throw new BaseFrameworkException("Condition is invalid.");
                }
            ...
            public T[] GetDataRows(Comparison<T> comparison)
            {
                if (comparison == null)
                {
                    throw new BaseFrameworkException("Comparison is invalid.");
                }
            ...
            public override bool AddDataRow(string dataRowString, object userData)
            {
                ...
                catch (Exception exception)
                {
                    ...
                    throw new BaseFrameworkException(Utility.Text.Format("Can not parse data row string for data table '{0}' with exception '{1}'.", new TypeNamePair(typeof(T), Name), exception), exception);
                }
            }
            ...
            public override bool AddDataRow(byte[] dataRowBytes, int startIndex, int length, object userData)
            {
                ...
                catch (Exception exception)
                {
                    ...
                    throw new BaseFrameworkException(Utility.Text.Format("Can not parse data row bytes for data table '{0}' with exception '{1}'.", new TypeNamePair(typeof(T), Name), exception), exception);
                }
            }
            ...
            private void InternalAddDataRow(T dataRow)
            {
                ...
                {
                    throw new BaseFrameworkException(Utility.Text.Format("Already exist '{0}' in data table '{1}'.", dataRow.Id, new TypeNamePair(typeof(T), Name)));
                }
