...
namespace BaseFramework.DataTable
{
    ...
    internal sealed partial class DataTableManager : BaseFrameworkModule, IDataTableManager
    {
        ...
        public void SetDataTableHelper(IDataTableHelper dataTableHelper)
        {
            ...
            {
                throw new BaseFrameworkException("Data table helper is invalid.");
            }
        ...
        public bool HasDataTable(Type dataRowType)
        {
            if (dataRowType == null)
            {
                throw new BaseFrameworkException("Data row type is invalid.");
            }

            if (!typeof(IDataRow).IsAssignableFrom(dataRowType))
            {
                throw new BaseFrameworkException(Utility.Text.Format("Data row type '{0}' is invalid.", dataRowType.FullName));
            }
        ...
        public IDataTable<T> CreateDataTable<T>(string name) where T : class, IDataRow, new()
        {
            ...
            {
                throw new BaseFrameworkException(Utility.Text.Format("Already exist data table '{0}'.", typeNamePair));
            }
        ...
        public bool DestroyDataTable<T>(IDataTable<T> dataTable) where T : IDataRow
        {
            if (dataTable == null)
            {
                throw new BaseFrameworkException("Data table is invalid.");
            }
