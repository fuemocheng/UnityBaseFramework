...
namespace XGame.Editor.DataTableTools
{
    public sealed partial class DataTableProcessor
    {
        ...
        public DataTableProcessor(string dataTableFileName, Encoding encoding, int nameRow, int typeRow, int? defaultValueRow, int? commentRow, int contentStartRow, int idColumn)
        {
            ...
            {
                throw new BaseFrameworkException("Data table file name is invalid.");
            }

            if (!dataTableFileName.EndsWith(".txt", StringComparison.Ordinal))
            {
                throw new BaseFrameworkException(Utility.Text.Format("Data table file '{0}' is not a txt.", dataTableFileName));
            }

            if (!File.Exists(dataTableFileName))
            {
                throw new BaseFrameworkException(Utility.Text.Format("Data table file '{0}' is not exist.", dataTableFileName));
            }
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                {
                    throw new BaseFrameworkException(Utility.Text.Format("Data table file '{0}', raw Column is '{2}', but line '{1}' column is '{3}'.", dataTableFileName, i, rawColumnCount, rawValue.Length));
                }
            ...
            if (nameRow < 0)
            {
                throw new BaseFrameworkException(Utility.Text.Format("Name row '{0}' is invalid.", nameRow));
            }

            if (typeRow < 0)
            {
                throw new BaseFrameworkException(Utility.Text.Format("Type row '{0}' is invalid.", typeRow));
            }

            if (contentStartRow < 0)
            {
                throw new BaseFrameworkException(Utility.Text.Format("Content start row '{0}' is invalid.", contentStartRow));
            }

            if (idColumn < 0)
            {
                throw new BaseFrameworkException(Utility.Text.Format("Id column '{0}' is invalid.", idColumn));
            }

            if (nameRow >= rawRowCount)
            {
                throw new BaseFrameworkException(Utility.Text.Format("Name row '{0}' >= raw row count '{1}' is not allow.", nameRow, rawRowCount));
            }

            if (typeRow >= rawRowCount)
            {
                throw new BaseFrameworkException(Utility.Text.Format("Type row '{0}' >= raw row count '{1}' is not allow.", typeRow, rawRowCount));
            }

            if (defaultValueRow.HasValue && defaultValueRow.Value >= rawRowCount)
            {
                throw new BaseFrameworkException(Utility.Text.Format("Default value row '{0}' >= raw row count '{1}' is not allow.", defaultValueRow.Value, rawRowCount));
            }

            if (commentRow.HasValue && commentRow.Value >= rawRowCount)
            {
                throw new BaseFrameworkException(Utility.Text.Format("Comment row '{0}' >= raw row count '{1}' is not allow.", commentRow.Value, rawRowCount));
            }

            if (contentStartRow > rawRowCount)
            {
                throw new BaseFrameworkException(Utility.Text.Format("Content start row '{0}' > raw row count '{1}' is not allow.", contentStartRow, rawRowCount));
            }

            if (idColumn >= rawColumnCount)
            {
                throw new BaseFrameworkException(Utility.Text.Format("Id column '{0}' >= raw column count '{1}' is not allow.", idColumn, rawColumnCount));
            }
        ...
        public bool IsIdColumn(int rawColumn)
        {
            ...
            {
                throw new BaseFrameworkException(Utility.Text.Format("Raw column '{0}' is out of range.", rawColumn));
            }
        ...
        public bool IsCommentRow(int rawRow)
        {
            ...
            {
                throw new BaseFrameworkException(Utility.Text.Format("Raw row '{0}' is out of range.", rawRow));
            }
        ...
        public string GetString(int index)
        {
            ...
            {
                throw new BaseFrameworkException(Utility.Text.Format("String index '{0}' is out of range.", index));
            }
        ...
        public bool GenerateDataFile(string outputFileName)
        {
            ...
            {
                throw new BaseFrameworkException("Output file name is invalid.");
            }
        ...
        public bool GenerateCodeFile(string outputFileName, Encoding encoding, object userData = null)
        {
            ...
            {
                throw new BaseFrameworkException("You must set code template first.");
            }
