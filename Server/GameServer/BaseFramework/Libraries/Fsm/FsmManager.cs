...
namespace BaseFramework.Fsm
{
    ...
    /// 有限状态机管理器。
    /// </summary>
    internal sealed class FsmManager : BaseFrameworkModule, IFsmManager
    {
        ...
        public bool HasFsm(Type ownerType)
        {
            if (ownerType == null)
            {
                throw new BaseFrameworkException("Owner type is invalid.");
            }
        ...
        public IFsm<T> CreateFsm<T>(string name, T owner, params FsmState<T>[] states) where T : class
        {
            ...
            if (HasFsm<T>(name))
            {
                throw new BaseFrameworkException(Utility.Text.Format("Already exist FSM '{0}'.", typeNamePair));
            }
        ...
        public bool DestroyFsm<T>(IFsm<T> fsm) where T : class
        {
            if (fsm == null)
            {
                throw new BaseFrameworkException("FSM is invalid.");
            }
